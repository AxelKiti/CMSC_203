...
public class TwoDimArrayPractice extends JFrame
{
   ...
   */
   public void fillValues( )
   {
    ...
    for ( int row = 0; row < intArray.length; row++ )
    {
        ...
        for ( int column = 0; column < intArray[row].length; column++ ) //while column number < current row's length, move to next column on said row
        {
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int row = 0;
	   int minimum = intArray[row+1][column];
	   //System.out.print( row + "\t" );
       for (row = 0; row <= 3; row++ ) //while column number < current row's length, move to next column on said row
       {
    	   if(intArray[row][column] < minimum) //if the value in the current row is less than the minimum...
    	   {
    		   minimum = intArray[row][column]; //then the new minimum will be the value in the current row
    	   }
